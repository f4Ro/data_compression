...
class UnMaxPoolWithArgmax(layers.Layer):
    ...
    def call(self, inputs, mask, stride=2):
        ksize = [1, stride, 1, 1]
        ...
        feature_range = tf.range(output_shape[3], dtype=tf.int32)  # range = 1 - due to one dimensional data
        f = one_like_mask * feature_range  # as feature_range is 1, this doesnt change anythong
        # transpose indices & reshape update values to one dimension
        updates_size = tf.size(inputs)
        # stacked = tf.stack([b, y, x, f]) # (tf.stack([[1,1,120,1], y, x, f]) >> output - 120 x 4
        indices = tf.transpose(tf.reshape(tf.stack([b, y, x, f]), [4, updates_size]))
        values = tf.reshape(inputs, [updates_size])  # Flatten
        # Put [values] at [indices] of an empty tensor with shape [shape]
