...
class CustomCallback(keras.callbacks.Callback):
    def __init__(
        ...
        if self.training_data is not None or self.validation_data is not None:
            if batch_size is None:
                raise Exception('batch_size may not remain unspecified if training or validation data is given')
    ...
    def on_epoch_end(self: CustomCallback, epoch: int, logs: dict) -> None:
        if epoch % self.epoch_interval == 0:
    ...
    def _update_time(self: CustomCallback) -> str:
        ...
        if self.last_timestamp is not None:
            ...
            return_val = f'{seconds_str}.{micros_str}'
    ...
    def print_results(
            self: CustomCallback, epoch: int, loss: float, improvement: str, percentage: float, time: str) -> None:
...
#     epochs = [0, 1, 2, 3, 4]
#     losses = [0.0056587159633636475, 0.0009753100457601249, 0.000884634384419769, 0.000861109234392643,
# 0.0008480784017592669]
